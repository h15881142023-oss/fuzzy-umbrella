# KPI 待办进度 / Todo 周报（周一/周四 14:00）— 已迁移本机

> **推荐**：本机 launchd，见 `scripts/LOCAL_AUTOMATIONS.md` 与 `scripts/run_kpi_todo_local.sh`。  
> 请停用 Cursor Cloud Automation「川藏一区Todo周报」。

以下步骤保留作排查参考。筛选 **川藏一区 + 本月**，生成标红表格图片并推送企业微信。

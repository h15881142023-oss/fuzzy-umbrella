"""非餐 KPI：优先读今日 dashboard 快照，否则走 CDP 抓取。"""
...
def main() -> int:
    db.init_db()
    cfg = load_endpoints()
    keywords = (cfg.get("network_keywords") or {}).get("non_catering", [])
    ...
    if payloads:
        ...
        try:
            ...
            msg = str(exc)
            write_status("sync_non_catering_scores", {"ok": False, "error": msg})
            db.log_sync("sync_non_catering_scores", "fail", msg)
            print(msg)
            return 1
    if not payloads:
        msg = "无非餐 KPI 数据。请先运行 scrape_dashboard_cdp.py 或配置看板 URL"
        write_status("sync_non_catering_scores", {"ok": False, "message": msg})
        db.log_sync("sync_non_catering_scores", "fail", msg)
        print(msg)
        return 1
    n = save_kpi_scores(payloads, cfg, "non_catering")
    write_status("sync_non_catering_scores", {"ok": True, "rows": n, "logs": logs})
    db.log_sync("sync_non_catering_scores", "ok", f"写入 {n} 行")
    print(f"非餐 KPI 写入 {n} 行")
    return 0
